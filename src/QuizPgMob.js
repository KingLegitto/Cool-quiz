...
const QuizPgMob = ({genre, bg, questions, setActivate,diff, set_Diff, dd, diffAnchor}) => {
    ...
    const redo = () =>{
        document.querySelector('.redo').style.animation = 'pickMob 0.7s ease 0s 1 normal forwards'
        set_Diff(parseInt(diffAnchor))
    ...
    const check = (choosen) => {
        ...
        if(choosen == questions[Diff][i].a){
            ...
            switch(choosen){
                case a:{
                    set_a('CORRECT!')
                    document.querySelectorAll('.ansBox')[0].classList.add('GverdictMob')
                    break}
                case b:{
                    set_b('CORRECT!')
                    document.querySelectorAll('.ansBox')[1].classList.add('GverdictMob')
                    break}
                case c:{
                    set_c('CORRECT!')
                    document.querySelectorAll('.ansBox')[2].classList.add('GverdictMob')
                    break}
                case d:{
                    set_d('CORRECT!')
                    document.querySelectorAll('.ansBox')[3].classList.add('GverdictMob')
                    break}
            }
            ...
            setTimeout(()=>{
                ...
                document.querySelectorAll('.ansBox').forEach((item)=>{item.classList.remove('GverdictMob')})
                while(true){
            ...
            <div className="w-[100%] relative bg-white quiz" style={{opacity: 0, transition: '1s',height: '100vh', overflowY: 'hidden'}}>
                ...
                backgroundColor: 'transparent', zIndex:'5'}}>
                    <div className=" h-[80%] ansBoxParent">
                        <button className="   rounded-[10px] shadow-2xl ansBox  centerText" onClick={()=>{check(a)}} >
                            {a}
                        </button>
                        <button className="   rounded-[10px] shadow-2xl ansBox centerText" onClick={()=>{check(b)}}>
                            {b}
                        </button>
                        <button className="   rounded-[10px] shadow-2xl ansBox centerText" onClick={()=>{check(c)}}>
                            {c}
                        </button>
                        <button className="   rounded-[10px] shadow-2xl ansBox centerText" onClick={()=>{check(d)}}>
                            {d}
                        </button>
                    </div>
